...
$sql_id = select('*', 'tb_registrasi', "id = $id");
...
<div class="row">
	<div class="col-md-4 col-md-offset-4">
		<form action="" method="post" class="form-group">
			...
			<input type="text" name="id" class="form-control" placeholder="Nomor Identitas" readonly="on" value="<?= $data->id; ?>">
			<br>

			<label for="nopol">Nomor Polisi</label>
			<input type="text" name="nopol" class="form-control" placeholder="Nomor Polisi" value="<?= $data->nopol; ?>">
			<br>
			...
			<input type="text" name="barang" class="form-control" placeholder="Barang" value="<?= $data->barang; ?>">
			<br>
			...
			<div class="row">
				<div class="col-md-6">
					...
					<div class="input-group date" id="time-picker1">
						<input type="text" name="jam_masuk" class="form-control" placeholder="MM:ss" value="<?= $data->jam_masuk; ?>">
				...
				<div class="col-md-6">
					...
					<div class="input-group time" id="time-picker2">
						<input type="text" name="jam_keluar" class="form-control" placeholder="MM:ss" value="<?= $data->jam_keluar; ?>">
						<span class="input-group-btn">
...
if (isset($_POST['submit'])) {
	...
	if (empty(trim($id)) || empty(trim($nama_lengkap)) || empty(trim($type)) || empty(trim($nopol)) || empty(trim($barang)) || empty(trim($jam_masuk)) || empty(trim($jam_keluar))) {
		echo alert('Form tidak boleh ada yang kosong!', '');
	} else {
		$insert = update('tb_registrasi', "nopol = '$nopol', nama_lengkap = '$nama_lengkap', type = '$type', barang = '$barang', jam_masuk = '$jam_masuk', jam_keluar = '$jam_keluar'", $id);
		if ($insert) {
			echo alert('Edit Data registrasi Truk berhasil!', '?page=registrasi');
		} else {
			echo alert('Edit Data registrasi Truk gagal!', '');
		}
	}
}
